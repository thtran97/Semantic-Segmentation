...
#         else : 
        self.output = tf.layers.conv2d(self.conv9_1,filters=self.n_classes,
                                  kernel_size = 1,
                                  strides=1,
                                  padding = "SAME",
...
#             self.loss_op = tf.reduce_mean(ul.f_loss(tf.cast(self.y,tf.float32),self.output,self.config.loss),name="fcn_loss")
            ...
            with tf.control_dependencies(update_ops): 
                if self.config.learning_rate == "default" : 
...
#             self.accuracy = tf.reduce_mean(um.f_accuracy(tf.cast(self.y_flatten,tf.float32),self.output_flatten,self.config.accuracy),name="accuracy")
